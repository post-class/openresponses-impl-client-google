...
さらに、同じ `GeminiResponsesClient` インスタンス内では、解決済みの
`system_instruction` もキャッシュされます。follow-up ターンで
`instructions/system/developer` が省略された場合は前回の値を再利用し、
後続ターンで新しい明示値が来た場合は追記ではなく置換します。
...
- 有効な `system_instruction` は `contents` とは別キャッシュで保持されるため、follow-up でもフル履歴再送なしで前回の指示を維持できます。
