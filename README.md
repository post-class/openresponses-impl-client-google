...
For `input_image`, `input_file`, and `input_video`:

- `data:` URIs are decoded and sent as inline bytes
- other URIs are sent as URI-based parts
- MIME type is guessed from the URI or filename
- fallback MIME type is `application/octet-stream`
